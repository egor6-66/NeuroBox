...
class RefusalName(StrEnum):
    ...
    TOOL_ABANDONED = "tool-abandoned"
    """Агент позвал ручку потребителя, и ответа на неё не дождался.

    Исполняет такую ручку сам потребитель у себя, а мы только передаём вызов и ждём. Ушёл он —
    ждать больше некого: результат не придёт никогда, а агент тем временем занят и стоит денег.
    """
