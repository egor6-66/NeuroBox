...
export class CliSession {
  ...
  static async start(spec: SessionSpec): Promise<CliSession> {
    ...
    // Встроенный набор ПУСТ. Агент получает ровно то, что дал рецепт, и ничего сверх.
    //
    // `--allowedTools` этого не делал и не мог: он управляет разрешениями, а не наличием.
    // Замер в контейнере показал 30 инструментов против одного рецептного — агенту были
    // доступны `Bash`, `Write`, `Edit`, `WebFetch`, `Task` и остальные, и часть он звал сам:
    // `ToolSearch` уходил на каждом прогоне, тратя время и деньги ни на что.
    //
    // Закрыто ЦЕЛИКОМ, а не списком запретов: список пришлось бы догонять за каждым
    // обновлением CLI, и однажды новый инструмент приехал бы открытым. Понадобится встроенный
    // — это отдельное осознанное решение, а не побочный эффект.
    args.push("--tools", "");
